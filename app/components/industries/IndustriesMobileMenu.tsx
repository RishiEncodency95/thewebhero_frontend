...
import { ChevronDown, ChevronRight, X, Building2, ArrowRight } from 'lucide-react';
import { INDUSTRY_CATEGORIES } from '../../lib/industries';

type IndustriesMobileMenuProps = {
    isOpen: boolean;
    onClose: () => void;
};

export default function IndustriesMobileMenu({ isOpen, onClose }: IndustriesMobileMenuProps) {
            ...
            <div className="relative w-full max-w-sm bg-white h-full flex flex-col shadow-2xl z-10 animate-in slide-in-from-right duration-300">
                <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 bg-slate-50">
                    <div className="flex items-center gap-2 font-black text-slate-900 text-base">
                        ...
                        <span>Industries Directory</span>
                    </div>
                            ...
                            >
                                ...
                                {isExpanded && (
                                    <div className="bg-white border-t border-slate-200/60 p-3 space-y-2">
                                        ...
                                        >
                                            View {cat.name} Sector Overview →
                                        </Link>

                                        {cat.industries.map((sub) => (
                                            <Link
                                                key={sub.slug}
