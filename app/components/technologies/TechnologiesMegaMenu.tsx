...
type TechnologiesMegaMenuProps = {
    isOpen: boolean;
    onClose: () => void;
};

export default function TechnologiesMegaMenu({ isOpen, onClose }: TechnologiesMegaMenuProps) {
                ...
                <div className="grid grid-cols-12 gap-8">
                    ...
                    <div className="col-span-4 border-r border-slate-100 pr-6 space-y-1">
                        <div className="flex items-center justify-between px-3 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                            ...
                            <span>{TECH_CATEGORIES.length} Categories</span>
                        </div>
                    ...
                    {/* Right Column: Active Category Technologies */}
                    <div className="col-span-8 flex flex-col justify-between pl-2">
                        <div>
                            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                                <div>
                                    ...
                                    <h3 className="text-xl font-black text-slate-900">
                                        <Link
                                            href={`/technologies/${activeCategory.slug}`}
                                            onClick={onClose}
                            ...
                            <div className="grid grid-cols-2 gap-3 max-h-[340px] overflow-y-auto pr-2 custom-scrollbar">
                                {activeCategory.technologies.map((sub) => (
                                    <Link
                                        key={sub.slug}
                        ...
                        <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between bg-slate-50 rounded-xl px-5 py-3">
                            <div className="flex items-center gap-2 text-xs text-slate-600 font-semibold">
                                ...
                                <span>Looking for full engineering technology stacks?</span>
                            </div>
                            ...
                            >
                                <span>View All Technologies Directory</span>
                                <ArrowRight className="h-3.5 w-3.5" />
                            </Link>
                        </div>
                    </div>
