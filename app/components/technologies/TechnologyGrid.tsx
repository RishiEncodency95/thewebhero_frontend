...
import { TechnologyCategoryGroup } from '../../types/technology';
import TechnologyCategory from './TechnologyCategory';
import { Search } from 'lucide-react';

type TechnologyGridProps = {
    categories: TechnologyCategoryGroup[];
};

export default function TechnologyGrid({ categories }: TechnologyGridProps) {
    ...
    const filteredCategories = categories
        .map((cat) => {
            ...
            const matchingTechnologies = cat.technologies.filter(
                (t) =>
                    t.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                    t.shortDescription.toLowerCase().includes(searchQuery.toLowerCase())
            );

            if (matchingTechnologies.length === 0 && searchQuery) {
                return null;
            }

            return {
                ...cat,
                technologies: searchQuery ? matchingTechnologies : cat.technologies,
            };
        })
        .filter(Boolean) as TechnologyCategoryGroup[];
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-14 space-y-6">
                    <div className="relative max-w-xl mx-auto">
                        ...
                        <input
                            type="text"
                            placeholder="Search tech stack (e.g. React, Next.js, Flutter, AWS, Docker, AI, Python...)"
                            value={searchQuery}
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                        ...
                        >
                            All Tech Stack ({categories.length})
                        </button>
                        {categories.map((cat) => (
                            <button
                                key={cat.slug}
                ...
                {filteredCategories.length > 0 ? (
                    <div className="space-y-20">
                        {filteredCategories.map((cat) => (
                            <TechnologyCategory key={cat.slug} category={cat} />
                        ))}
                    </div>
                ) : (
                    <div className="text-center py-16 rounded-2xl border border-dashed border-slate-300 bg-slate-50">
                        <p className="text-lg font-bold text-slate-700">No technologies match &quot;{searchQuery}&quot;</p>
                        <p className="text-sm text-slate-500 mt-1">Try searching for keywords like React, Node, Cloud, or Docker.</p>
                        <button
                            onClick={() => {
                                setSearchQuery('');
